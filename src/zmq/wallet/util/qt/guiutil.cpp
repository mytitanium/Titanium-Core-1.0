...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert ttm:// to ttm:
    //
    //    Cannot handle this later, because ttm:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ttm://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "ttm:");
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Titanium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Titanium (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Titanium (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Titanium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Titanium app to startup item list
